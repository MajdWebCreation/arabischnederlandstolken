-- Atomically turns a draft into an issued invoice: validates required
-- billing information, assigns the next sequential invoice number for the
-- current year (race-safe, same atomic-upsert principle as
-- set_booking_number() in 20260814100400_bookings.sql - never a
-- `select max(...) + 1`), fills in invoice_date/due_date if still unset,
-- and freezes the seller/customer snapshot actually printed. Everything
-- happens in one statement-level transaction, so there is no window where
-- an invoice has a number but no snapshot, or a snapshot but no number.
...
begin
  ...
  if v_customer.billing_address is null or btrim(v_customer.billing_address) = '' then
  ...
  set
    ...
    customer_snapshot = jsonb_build_object(
      ...
      'address', v_customer.billing_address,
...
comment on function public.issue_invoice(uuid) is
  'Admin-only. Validates billing info and line items, assigns the next race-safe sequential invoice number for the current year, freezes seller/customer snapshots, and moves status draft -> issued, all atomically. invoice_issued is logged automatically by log_invoice_changes_trigger.';
