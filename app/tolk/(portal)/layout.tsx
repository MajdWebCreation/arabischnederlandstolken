...
export default async function InterpreterPortalLayout({
  children,
      ...
      <div className="flex min-h-screen items-center justify-center px-4 py-16">
        <div className="panel max-w-md px-8 py-9 text-center">
          ...
          <h1 className="mt-3 text-xl font-semibold text-foreground">
            Dit account heeft geen toegang tot het tolkenportaal
          </h1>
          <p className="mt-3 text-sm leading-7 text-muted">
            U bent ingelogd als {session.user.email}, maar dit account is
            niet (meer) gekoppeld aan een actief tolkprofiel. Neem contact op
            met Arabisch Nederlands Tolken als u hier toegang toe zou moeten
            hebben.
          </p>
          <form action={signOut.bind(null, "/tolk/login")} className="mt-6">
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            >
              ...
              <span className="text-sm font-semibold text-foreground">
                {session.interpreter.first_name}
              </span>
            </Link>
